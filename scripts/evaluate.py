...
METRIC_RE = re.compile(r"(categorical_accuracy|action_accuracy|rule_violation_rate)['\":= ]+([0-9]*\.?[0-9]+)")


def parse_args():
    parser = argparse.ArgumentParser(description="Summarize BabyAI NeuPSL runtime output and text logs.")
...
def main() -> None:
    ...
    result_dir = args.results_dir or build_result_dir(
        DatasetConfig(name=args.dataset, train_size=args.train_size, valid_size=args.valid_size, inference_size=args.inference_size),
        args.results_root,
    )
